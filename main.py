...
Label(Titre, image=photo_taille, bg="black").pack()
...
Label(deco, image=photodeco_taille, bg="black").pack(side=LEFT)
...
diff = Label(Frame4, bg="green", fg="white")
diff.pack(pady=5)
valeur = DoubleVar()
scale1 = Scale(Frame4,from_ = 1, to = 10, variable = valeur ,orient = HORIZONTAL, resolution = 1, cursor="pencil",bd=0, bg="black", fg="white")
scale1.pack(pady=10, padx=23)
Button(Frame4, text="C'est partit!", command = épaisseur).pack(padx=10,pady=5)

#___couleur du tracé 

Label(Frame43,text="De quelle couleur\nsera votre tracé?",bg="black", fg="white").pack(padx=20,pady=5)
Button(Frame43,text="Définir ma couleur", command=couleurfond, cursor="pencil").pack(padx=10,pady=5)

#___Résultat

Label(Frame42,text="/!\Attention/!\ \n Le tracé débute dès que la couleur sélectionné est détectée, retournez la spatule lorsque vous êtes prêts.",bg="black", fg="white").pack(padx=5,pady=10)
"""Fin = Canvas(Frame42,width = 50, height =50, bg="black", bd=0)
Fin.pack(padx=3,pady=3)"""
Button(Frame42,text="réaliser un nouveau tracé", command=Fermer, cursor="pencil").pack(padx=10,pady=10)
